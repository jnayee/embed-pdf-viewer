...
<Callout type="info" title="Checkbox and Radio Button Values">
  For checkboxes and radio buttons, the value must match the exact **export value** defined in the PDF (e.g. `"Yes"`, `"Off"`, `"Choice1"`). If a checkbox has an export value of `"Yes"`, passing `"Yes"` will check the box, and `"Off"` will uncheck it.
</Callout>
...
The example below displays a PDF form on the left and a live JSON view of all field values on the right. Try filling in the fields to see the state update in real time.
...
| **`selectField(annotationId, options?, documentId?)`** | Focuses a specific field. Pass `{ scroll: true }` to scroll it into view. |
| **`deselectField(documentId?)`** | Removes focus from the currently selected field. |
| **`selectNextField(options?, documentId?)`** | Moves focus to the next field in tab order. |
| **`selectPreviousField(options?, documentId?)`** | Moves focus to the previous field in tab order. |
